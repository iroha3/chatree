...
| `bun test:ux` | Edge:9222 + dev:5175 | 49 项节点交互：去重、思考折叠、连续阅读（整条路径 + 分支切换 + 读到底翻页）、星标三态、删除确认+撤销、排版、代码块等宽、编辑态进出、动作按钮的位置/尺寸/右对齐、滚轮滚动链（内层/画布分流）、设置面板无 max tokens、标签页标题跟随语言… |
