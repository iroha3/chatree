...
- **卡片右下角一列“悬浮簇”**：停止生成 / 重新生成分支 / 复制 / 展开阅读，
  `opacity-0 group-hover:opacity-100`，平时完全不占视觉空间：

  | 状态 | 簇里包含的动作 |
  |---|---|
  | 流式生成中 | 停止生成（`Square` → `onStop`） |
  | 空闲且有回答（或报错） | 重新生成分支（`RefreshCcw` → `onRetry`，**另起兄弟分支**） |
  | 空闲且有回答 | 复制、展开阅读 |
...
- Ctrl/Cmd+Enter 也能发送。
- **`textarea` 上 `resize-none` 和 `block` 两个 class 必须保留：**
  - `resize-none`：避免浏览器原生缩放手柄与绝对定位的发送按钮产生视觉重叠与交互冲突（`rows={3}` 固定行高，无需手动缩放）。
  - `block`：`textarea` 默认 `inline-block` 会给父层底部留一道基线空隙（约 6.5px），导致绝对定位的按钮“沉”到边框上（实测只剩 0.3px）。
...
### 3.8 模型参数
- **节点卡片上不设置「最大令牌数」控件**（对话节点、系统节点均无）。
  单节点滑块粒度粗且容易误触，统一在「设置 → 模型」配置**模型默认值**。
- 「模型」面板中采用 `type="number"`：输入过程中不设实时夹逼，失焦时规范至 `[256, 65535]`。
- `node.maxTokens` 字段保留以兼容旧数据，新节点默认从模型继承配置；`onMaxTokensChange` 保留在 `NodeData` 接口中。

### 3.9 界面宽度与排版
- **宽度对齐**：`.node-content` 的宽度（`src/index.css`）必须与 `ChatFlow.tsx` 的 `NODE_WIDTH` 严格一致（当前 **516**），避免节点与连线出现锚点错位。
- **正文渲染**：回答正文由 `md-editor-rt` 渲染，字号与间距集中于 `src/index.css` 的 `.md-preview`（卡片与阅读浮窗共用）：卡片正文 19px，紧凑段落与列表间距；阅读浮层追加 `.reader` 降至 16px。
- **代码块等宽无缝**：标题栏 `.md-editor-code-head` 与代码区域必须严格等宽且垂直无缝隙。`.md-preview .md-editor-code pre` 内边距强制归零，缩进由内部 code 自身控制，行号不受影响。回归校验见 `node-ux-check.mjs`。
...
- 右侧那一栏就是**画布上横向的兄弟分支**，点一下就切过去。**切换的语义 = 把阅读终点
  ...
  理由见 D-013。
- **打开时定位到目标卡片的顶部**（双击 = 「从这里开始读」），不是卡片中间：`block: 'start'`。
  滚轮翻页和点分支用**同一种平滑滚动**；只有首屏（浮层刚出现那一下）用瞬到。
  > ⚠️ 别给滚轮翻页用 `behavior: 'auto'`。曾经为了「怕和平滑滚动打架」这么写过，
  > 结果内容直接传送走、读者当场丢掉位置感（用户：「我甚至分不清我看到哪里了」）。
  > 真起冲突的地方不是滚动本身，而是**平滑滚动起步前那几十毫秒容器还在底部**、
  > 会被判成「又到底了」再翻一次 —— 用 `advanceLockRef` 挡住就行。
- 翻到（或点开）的那张卡片有**落点高亮**（`.reader-land`，1.2s 淡出的光晕，见 `index.css`）。
- **读到底再继续滚 = 翻到下一轮**，复用画布上「卡片滚到底、画布接着走」的同一套手感。
  两个约束：
  - **只有恰好一个后续时才自动翻**；有分叉就只给按钮、让用户自己选（不能替他猜，见 D-013）；
  - 要**多滑一截**（`ADVANCE_AFTER = 140`）才翻，否则最后一行还没读完就被顶走。
  判定「读到头」直接复用 `wheelChain.ts` 的 `cardConsumesWheel()` —— 只要还有任何一层
  （比如思考过程的长文本）能滚，就不算到头。
